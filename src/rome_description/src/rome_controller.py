...
# Issue commands to the rome motors to achieve the target velocity
# Use a PID that compares the error based on encoder readings
class ControlsToMotors:
  def __init__(self):
    rospy.init_node('rome_controller')
    ...
    self.rome_on = rospy.get_param('~rome_on',False)
    if self.rome_on:
      import rome
      import atexit
      atexit.register(rome.stop)
  ...
  def motorcmd_2_robot(self, wheel='left', motor_command=0):
    if self.rome_on:
      ...
      import rome
      if wheel == 'left':
        if motor_command >= 0: rome.motor1(1,motor_command_raw)
        elif motor_command < 0: rome.motor1(0,motor_command_raw)
      if wheel == 'right':
        if motor_command >= 0: rome.motor2(1,motor_command_raw)
        elif motor_command < 0: rome.motor2(0,motor_command_raw)
  ...
  def spin(self):
    rospy.loginfo("Start rome_controller")
  ...
  def shutdown(self):
    rospy.loginfo("Stop rome_controller")
  	# Stop message
