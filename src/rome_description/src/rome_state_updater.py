...
class WheelEncoderPublisher:
  def __init__(self):
    ...
    self.rome_on = rospy.get_param('~rome_on',True)
    if self.rome_on:
      import rome   
      self.lwheel_encs = [rome.enc_read(1)]*5
      self.rwheel_encs = [rome.enc_read(0)]*5
  ...
  def update(self):
    if self.rome_on: # Running on actual robot
      import rome
      lwheel_enc = self.lwheel_dir * rome.enc_read(1) * .01 # cm's moved
      rwheel_enc = self.rwheel_dir * rome.enc_read(0) * .01 # cm's moved
  ...
  def spin(self):
    rospy.loginfo("Start rome_state_updater")
  ...
  def shutdown(self):
    rospy.loginfo("Stop rome_state_updater")
    # Stop message
